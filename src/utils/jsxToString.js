...
import { is, isNil, sort, keys, range } from 'ramda';
import traverse from 'traverse';

const noop = () => {};
const collapseWhiteSpace = val => String(val).replace(/\s+/g, ` `);
const spacer = n => range(0, n).map(() => `  `).join(``);
const meaningfulChildren = c => {
	return c !== true && c !== false && c !== null && c !== ``;
};
...
function jsxToString({ ReactElement = null, ...options }) {
	if (is(String, ReactElement) || is(Number, ReactElement)) {
		return ReactElement;
	}
	if (!isElement(ReactElement)) {
		throw new Error('not a valid react/jsx element');
	}
...
function stringifyObj(any) {
	if (!Object.keys(any).length || !any.length) {
		return sanitizeObjectString(any);
	}
...
function formatJSXVal(val) {
	...
	if (is(Object, val) && !('length' in val)) { str = stringifyObj(val); }
	if (is(Object, val) && ('length' in val)) { str = sanitizeObjectString(val); }
